...
const ReplyForm = ({ noteId, onSubmit }: { noteId: string; onSubmit: (content: string) => Promise<void> }) => {
  ...
  return (
    <div className="mt-4 mb-6 pl-3 relative space-y-2">
      <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-border" style={{
        left: '0px'
      }} />
      <Textarea
...
export default function ThreadPage() {
  ...
  const NotePost = ({ note, level = 0, isTarget = false }: { note: INote | INoteReply; level?: number; isTarget?: boolean }) => {
    ...
    return (
      <div ref={noteRef} className={`relative ${level > 0 ? 'pl-3' : ''}`}>
        {level > 0 && (
          <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-border" style={{
            left: `${(level - 1) * 12}px`
          }} />
